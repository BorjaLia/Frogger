...
////////////////////////////////////////////////////////////
template <typename T>
constexpr Rect<T>::Rect(Vector2<T> thePosition, Vector2<T> theSize) : pos(thePosition), size(theSize)
{
}
...
{
    return Rect<U>(Vector2<U>(pos), Vector2<U>(size));
}
...
constexpr std::optional<Rect<T>> Rect<T>::findIntersection(const Rect<T>& rectangle) const
{
    ...
    const T r1MinX = min(pos.x, static_cast<T>(pos.x + size.x));
    const T r1MaxX = max(pos.x, static_cast<T>(pos.x + size.x));
    const T r1MinY = min(pos.y, static_cast<T>(pos.y + size.y));
    const T r1MaxY = max(pos.y, static_cast<T>(pos.y + size.y));

    // Compute the min and max of the second rectangle on both axes
    const T r2MinX = min(rectangle.pos.x, static_cast<T>(rectangle.pos.x + rectangle.size.x));
    const T r2MaxX = max(rectangle.pos.x, static_cast<T>(rectangle.pos.x + rectangle.size.x));
    const T r2MinY = min(rectangle.pos.y, static_cast<T>(rectangle.pos.y + rectangle.size.y));
    const T r2MaxY = max(rectangle.pos.y, static_cast<T>(rectangle.pos.y + rectangle.size.y));
